...
const DashboardSitesEdit = () => {
  ...
  const fetchDataSite = async () => {
    try {
      ...
      setSelectedCategories(selectSite.categories || []); // ✅ جلب كل الفئات
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://www.betbonus24.com/api/admin/upload/site-logo/",
        formData,
        {
          headers: {
      ...
      <div>
        ...
        <div className='items-center justify-center text-center'>
          ...
          <form className='mb-8' onSubmit={handleUpdateSite}>
            ...
            {/* Categories */}
            <div className="flex flex-col  mb-1 mr-4 px-4 ">
              <label className="font-extrabold mb-2 text-xl">الفئات</label>
                  ...
                  <button
                    ...
                    className={`px-4 py-1 font-semibold text-base rounded-md cursor-pointer transition-colors duration-300 ease-in-out
                      ${selectedCategories.some(cat => cat.id === item.id)
                        ...
                        : 'bg-[#009788] text-[#101B2D] hover:bg-[#00cc66] hover:text-white'}
                    `}
                  >
                    {item.name}
                  </button>
